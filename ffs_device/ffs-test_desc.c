...
static const struct strings_type {
...
void write_bin(char * buf, int size, char * name) {
	FILE *write_ptr;
...
{
	write_bin((char *)&descriptors, sizeof(struct strings_type), "descriptors-ffs-test.bin");
	write_bin((char *)&strings, sizeof(struct strings_type), "strings-ffs-test.bin");
}
